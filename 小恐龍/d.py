...
class MenuSystem:
    ...
    def draw(self, screen):
        ...
        # 繪製選單選項 - 動態位置和大小，增加間距
        start_y = int(SCREEN_HEIGHT * 0.25)  # 25% 的螢幕高度開始
        option_spacing = int(SCREEN_HEIGHT * 0.12)  # 12% 螢幕高度的間距，增加間距
    ...
    def update(self):
        ...
        if hasattr(self, "nightmare_effects"):
            ...
            if previous_gravity_state and not self.is_gravity_reversed:
                if not self.is_jumping and self.y < GROUND_HEIGHT - self.height:
                    self.is_jumping = True
                    self.jump_speed = 2  # 給一個向下的初始速度
...
class Game:
        ...
        if self.obstacle_timer <= 0:
            ...
            if is_gravity_reversed and random.randint(1, 3) == 1:
                ...
                self.obstacles.append(
                    Obstacle(obstacle_type="flying", colors=self.colors)
                )
            ...
            # 重力反轉時縮短生成間隔
            if is_gravity_reversed:
                base_interval = int(base_interval * 0.7)  # 減少30%間隔
            ...
            if not self.game_over:
                ...
                for obstacle in self.obstacles[:]:
                    ...
                    if obstacle.x + obstacle.width < 0:
                        ...
                        self.score += int(10 * score_multiplier.get(self.difficulty_level, 1))
    ...
    def draw_game_info(self):
        ...
        difficulty_color = self.colors["PURPLE"]
